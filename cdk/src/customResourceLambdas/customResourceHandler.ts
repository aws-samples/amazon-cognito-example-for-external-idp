...
export abstract class CustomResourceHandler<T extends { [key: string]: string | undefined }> {
  ...
  public handler = async (event: CloudFormationCustomResourceEvent, context: Context, callback: Callback): Promise<void> => {
    ...
    if (event.RequestType === "Create") {
      ...
      try {
        ...
        await this.sendResponse(event, context, "FAILED", {"Message": "Resource creation failed!", "Error": error.message});
      }
      ...
      try {
        ...
        console.error(error);
        await this.sendResponse(event, context, "FAILED", {"Message": "Resource deletion failed!", "Error": error.message});
      }
  ...
  private async sendResponse(event: CloudFormationCustomResourceEvent,
                             ...
                             responseData?: T | { "Message": string, "Error": string }): Promise<void> {
    try {
