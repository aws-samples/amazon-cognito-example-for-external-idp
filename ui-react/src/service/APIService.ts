...
export class HttpAPIService implements APIService {
    ...
    public async savePet(pet: Pet) {
        const authorizationHeader = await this.getAuthorizationHeader();
        if (pet.id) {
            await this.api.put(REST_API_NAME, `/pets/${pet.id}`, {body: pet, headers: authorizationHeader});
        } else {
            await this.api.post(REST_API_NAME, '/pets', {body: pet, headers: authorizationHeader});
        }
    }
    ...
    public async deletePet(pet: Pet): Promise<void> {
        const authorizationHeader = await this.getAuthorizationHeader();
        await this.api.del(REST_API_NAME, `/pets/${pet.id}`, {headers: authorizationHeader});
    }
    ...
    public async forceSignOut() {
        const authorizationHeader = await this.getAuthorizationHeader();
        try {
            await this.api.post('main', '/forceSignOut', {headers: authorizationHeader});
        } finally {
